...
def load_cfg(args):
    ...
    cfg_path = f'vid2player/cfg/**/{args.cfg}.yaml'
    ...
    cfg['env']['horizon_length'] = cfg_train['params']['config']['horizon_length']
    ...
    cfg_train['params']['config']['max_test_steps'] = args.max_test_steps

    os.makedirs(network_path, exist_ok=True)
    os.makedirs(log_path, exist_ok=True)

    cfg['env']['controller_cfg_name'] = args.cfg
    if args.test:
        cfg['name'] = cfg['test_name']
    cfg['env']['is_train'] = not args.test
    if args.test_motion_id is not None:
        cfg['env']['test_motion_id'] = args.test_motion_id
    
    cfg['env']['record'] = args.record
    cfg['env']['record_scenepic'] = args.record_scenepic
    ...
    if cfg_train['params']['config'].get('pretrained_model_cp', False):
        cfg['env']['load_pretrained'] = True
    
    if args.export_motion_file is not None:
        cfg['env']['export_motion_file'] = args.export_motion_file
        cfg_train['params']['config']['export_motion_file'] = args.export_motion_file
    
    cfg['env']['enable_shadow'] = args.enable_shadow
    cfg['env']['camera'] = args.camera

    if args.ball_traj_file_test is not None:
        cfg['env']['ball_traj_file_test'] = args.ball_traj_file_test

    cfg['env']['select_best'] = args.select_best
    cfg['env']['start_eg_id'] = args.start_eg_id
    cfg['env']['num_eg'] = args.num_eg
...
def parse_sim_params(args, cfg, cfg_train):
    ...
    if args.physics_engine == gymapi.SIM_FLEX:
        ...
        sim_params.physx.friction_offset_threshold = 0.02
...
def get_args(benchmark=False):
    custom_parameters = [
        ...
        {"name": "--cfg_dir", "type": str, "default": "vid2player", "help": "cfg folder"},
        ...
        {"name": "--test_motion_id", "type": int, "default": None},
        {"name": "--record", "action": "store_true", "default": False},
        {"name": "--record_scenepic", "action": "store_true", "default": False},
        {"name": "--record_kinematic", "action": "store_true", "default": False},
        {"name": "--num_rec_frames", "type": int, "default": 300},
        {"name": "--rec_fname", "type": str, "default": None},
        {"name": "--rec_once", "action": "store_true", "default": False},
        {"name": "--export_motion_file", "type": str, "default": None},
        {"name": "--enable_shadow", "action": "store_true", "default": False},
        {"name": "--ball_traj_file_test", "type": str, "default": None},
        {"name": "--max_test_steps", "type": int, "default": 27000},
        {"name": "--select_best", "action": "store_true"},
