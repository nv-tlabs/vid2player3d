...
class MjViewer(MjViewerBasic):
    ...
    def __init__(self, sim):
        ...
        self._video_idx = 0
        self._video_path = f"out/recording/video_%s.mp4"
        os.makedirs(os.path.dirname(self._video_path), exist_ok=True)

        # vars for capturing screen
        self._image_idx = 0
        self._image_path = f"out/screenshot/video_%s.mp4"
    ...
    def _read_pixels_as_in_window(self):
        ...
        # img = self.sim.render(*resolution).copy()
        img = self.read_pixels(*resolution, depth=False)[::-1, :, :]
    ...
    def key_callback(self, window, key, scancode, action, mods):
        ...
        # Advances simulation by one step.
            ...
            print(f"Writting image to {self._image_path % self._image_idx}")
            self._image_idx += 1
