...
def batch_get_pelvis_orient(rel_pose_skeleton, rel_rest_pose, parents, children, dtype):
    ...
    center_final_loc = torch.tensor(0, dtype=dtype, device=rel_pose_skeleton.device)
    center_rest_loc = torch.tensor(0, dtype=dtype, device=rel_pose_skeleton.device)
...
@torch.jit.script
def batch_rigid_transform_smpl(rot_mats, joints, parents):
    """
    Applies a batch of rigid transformations to the joints
    ...
    for indices in idx_levs[1:]:
        transforms[:, indices] = torch.matmul(
                transforms[:, parents[indices]], 
                transforms_mat[:, indices]
            )
...
def batch_get_pelvis_orient_svd(rel_pose_skeleton, rel_rest_pose, parents, children, dtype):
    ...
    # S_non_zero = S[mask_zero != 0]    # this will cause illegal memory access
    ind = torch.where(mask_zero != 0)[0]
    nind = torch.where(mask_zero == 0)[0]
    S_non_zero = S[ind]
    ...
    rot_mat[nind] = torch.eye(3, device=S.device)

    det_u_v = torch.sign(det3x3(torch.bmm(V.detach(), U.detach().transpose(1, 2))))
    # det_u_v[det_u_v == 0.0] = 1.0
...
def batch_get_3children_orient_svd(rel_pose_skeleton, rel_rest_pose, rot_mat_chain_parent, children_list, dtype):
    # type: (Tensor, Tensor, Tensor, List[int], int) -> Tensor
    rest_mat = []
    target_mat = []
...
# @torch.jit.script
def batch_inverse_kinematics_transform_old(
        pose_skeleton, global_orient,
        phis,
        rest_pose,
        children, parents,
        leaf_thetas):
    # type: (Tensor, Optional[Tensor], Tensor, Tensor, Tensor, Tensor, Tensor) -> Tensor
    """
...
def batch_inverse_kinematics_transform(
        ...
        rest_pose,
        children, parents,
        leaf_thetas, ignore_outlier):
    # type: (Tensor, Optional[Tensor], Tensor, Tensor, Tensor, Tensor, Tensor, bool) -> Tuple[Tensor, Tensor, Tensor]
    """
    Applies a batch of inverse kinematics transfoirm to the joints
    ...
    for idx_lev in range(1, len(idx_levs)):
        ...
        if idx_lev == len(idx_levs)-1:
            ...
            children_final_loc = final_pose_skeleton[:, spine_child] - rotate_rest_pose[:, [i]]
            children_rest_loc = rel_rest_pose[:, spine_child]

            rot_mat = batch_get_3children_orient_svd(
                children_final_loc.clone(), children_rest_loc.clone(),
                rot_mat_chain[:, parents[i]].clone(), spine_child, dtype)

            rot_mat_chain[:, i] = torch.matmul(
                rot_mat_chain[:, parents[i]].clone(),
                rot_mat
            )
            ...
            # (B, K, 3, 1)
            rel_pose_optimized = final_pose_skeleton[:, children[indices]] - rotate_rest_pose[:, indices]
            ...
            if ignore_outlier:
                ...
                orig_vec = orig_vec * norm_t / torch.norm(orig_vec, dim=2, keepdim=True)

                diff = torch.norm(rel_pose_optimized - orig_vec, dim=2, keepdim=True)
                big_diff_idx = torch.where(diff > 15 / 1000)[0]

                rel_pose_optimized[big_diff_idx] = orig_vec[big_diff_idx]
            ...
            # (B, K, 1, 1)
            child_final_norm = torch.norm(child_final_loc, dim=2, keepdim=True)     # \arrow{p_k}
            child_rest_norm = torch.norm(child_rest_loc, dim=2, keepdim=True)       # \arrow{t_k}
    ...
    return rot_mats, rot_mat_chain, rotate_rest_pose.squeeze(-1)
...
def batch_inverse_kinematics_transform_naive(
        ...
        children, parents,
        leaf_thetas):
    # type: (Tensor, Optional[Tensor], Tensor, Tensor, Tensor, Tensor, Tensor) -> Tuple[Tensor, Tensor]
    """
    Applies a batch of inverse kinematics transfoirm to the joints
    ...
    if global_orient is None:
        ...
        # assert(torch.all((global_orient_mat.norm(dim=-1) - 1.0).abs() < 1e-6))
    else:
    ...
    for idx_lev in range(1, len(idx_levs)):
        ...
        if idx_lev == len(idx_levs)-1:
            ...
            len_indices = len(indices)
            # (B, K, 3, 1)
            child_final_loc = torch.matmul(                                 # Eq (11) left side, rot_mat_chain[:, parents[indices]].transpose(2, 3) == R_pa(k)^{-1}
                    rot_mat_chain[:, parents[indices]].transpose(2, 3),
